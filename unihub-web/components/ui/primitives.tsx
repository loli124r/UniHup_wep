...
export function Card({ className, children, hover = true, ...props }: HTMLAttributes<HTMLDivElement> & { hover?: boolean }) {
  return (
    <motion.div
      ...
      className={cn(
        "rounded-card bg-bg-surface border border-border p-6 shadow-soft",
        className
      )}
      {...(props as any)}
    >
...
export const Input = forwardRef<HTMLInputElement, InputHTMLAttributes<HTMLInputElement>>(
  ({ className, ...props }, ref) => (
    <input
      ref={ref}
      className={cn(
        "w-full h-12 rounded-input border border-border bg-white px-4 text-sm text-text-primary placeholder:text-text-disabled outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20",
        className
      )}
      {...props}
    />
