...
export function useMyGradeForSubject(subjectId: string | undefined) {
  const { currentUser } = useAuth();
  const [result, setResult] = useState<SubjectGradeResult | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoading(true);
      const g = await fetchGradeFor(subjectId, currentUser.section, currentUser.studyType, currentUser.id);
      if (active) {
        setResult(g ? { subjectId, subjectName: "", sheet: g.sheet, record: g.record } : null);
        setLoading(false);
      }
    })();
    return () => {
  ...
  }, [subjectId, currentUser?.id, currentUser?.section, currentUser?.studyType]);
